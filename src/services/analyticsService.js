 ...
 * All queries include mandatory date filters to prevent OOM on large datasets
 */
class AnalyticsService {
     ...
     * 
     * @param {string} organizationId - Organization UUID
     * @param {Object} options - { days: number } — defaults to 30 days
     * @returns {Array} List of projects with spend
     */
    async getSpendByProject(organizationId, options = {}) {
        // Mandatory date filter — default 30 days to prevent OOM on large datasets
     ...
     */
    async getSpendByModel(organizationId, options = {}) {
        // Mandatory date filter — default 30 days to prevent OOM on large datasets
        const daysBack = options?.days || 30;
        const since = new Date();
        since.setDate(since.getDate() - daysBack);
     ...
     * 
     * @param {string} organizationId - Organization UUID
     * @param {number} days - Number of days to look back (max 90, default 30)
     * @returns {Array} Daily spend data
     */
    async getDailySpend(organizationId, days = 30) {
