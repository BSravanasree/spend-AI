    ...
    async recordAlert(params) {
        ...
        try {
            ...
            if (error) {
                ...
                logger.error(`Failed to record alert: ${error.message}`, 'ALERT_SERVICE');
                throw error;
            }

            logger.info(`${alert_level.toUpperCase()} budget crossed ${threshold_percent}% threshold: ${actual_spend}/${budget_amount}`, 'ALERT');
            return { success: true, data };
        } catch (error) {
            logger.error(`Alert recording error: ${error.message}`, 'ALERT_SERVICE');
