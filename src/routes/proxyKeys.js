...
router.get('/project/:projectId', async (req, res) => {
    try {
        ...
        logger.error(`Get proxy keys route error: ${error.message}`, 'PROXY_KEYS');
...
router.get('/project/:projectId/count', async (req, res) => {
    try {
        ...
        logger.error(`Get key count route error: ${error.message}`, 'PROXY_KEYS');
        return res.status(500).json({
            success: false,
...
router.get('/:id', async (req, res) => {
    try {
        ...
        logger.error(`Get proxy key route error: ${error.message}`, 'PROXY_KEYS');
...
router.post('/', requireAdmin, async (req, res) => {
    try {
        ...
        logger.error(`Create proxy key route error: ${error.message}`, 'PROXY_KEYS');
        return res.status(500).json({
            success: false,
 ...
 * Revoke (disable) a proxy key (Owner only — kills all traffic using this key)
 */
router.post('/:id/revoke', requireAdmin, async (req, res) => {
    try {
        ...
        logger.error(`Revoke proxy key route error: ${error.message}`, 'PROXY_KEYS');
